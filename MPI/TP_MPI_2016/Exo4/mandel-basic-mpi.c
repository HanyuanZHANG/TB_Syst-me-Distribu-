...
int main(int argc, char *argv[])
{
  ...
  if (rank == 0) {
    ...
    /* Begin User Program  - the master */
     //MPI_Recv(res, BLOCKLEN*NY, MPI_INT, 1, DATATAG, MPI_COMM_WORLD, &status);
         
     for(i = -MAXX; i <= MAXX; i++) {
        if(len<BLOCKLEN){
            for(j = -MAXY; j <= MAXY; j++) {
              cases[i+MAXX][j + MAXY] = res[len][j+MAXY];
            }
            len++;
        }else{
            MPI_Recv(res, BLOCKLEN*NY, MPI_INT, 1, DATATAG, MPI_COMM_WORLD, &status);
   ...
   for(i = -MAXX; i <= MAXX; i++) {
    ...
    double x, y;
    int i, j, res[BLOCKLEN][NY], rc;
    int send_len = 0;

    for(i = -MAXX; i <= MAXX; i++) {
        printf("value iiiiiiiiii %d\n", i);
        for(j = -MAXY; j <= MAXY; j++) {
            x = 2 * i / (double)MAXX;
            y = 1.5 * j / (double)MAXY;
            res[send_len][j+MAXY] = mandel(x, y);            
        }
        printf("value send_len %d\n", send_len);
        send_len ++;
        if(send_len==BLOCKLEN || i==MAXX){
            printf("avant send\n");
            MPI_Send(res, BLOCKLEN*NY, MPI_INT, 0, DATATAG, MPI_COMM_WORLD); 
            send_len = 0;
        }
     }
